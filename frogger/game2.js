	...
	if (level == 1){
		for (i = 0; i < 14; i++){
			...
			if ((Math.floor(i / 3) % 2) == 0){
				vehicles[i].dir = "left";
				vehicles[i].xcor = (405 + ((i % 3) * 75));
			} else {
				vehicles[i].dir = "right";
				vehicles[i].xcor = (-5 - ((i % 3) * 75));
			}
			if (i > 11){
				vehicles[i].speed = 1;
			}
