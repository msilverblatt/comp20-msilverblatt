...
function showStations(){
	...
	for (var d in data){
		stationKeys[d.PlatformKey] = d.StationName;
	}
...
function findTimes(station){
	var result = "";
	if (times.length > 0){
		result += "<table><tr><th>Direction</th><th>Time</th></tr>";
		for (var t in times){
			if (stationKeys[t.PlatformKey] == station){
				result += "<tr><td>NORTHBOUND</td><td>" + t.TimeRemaining + "</td></tr>";
			}
		}
		result += "</table>";
	}
